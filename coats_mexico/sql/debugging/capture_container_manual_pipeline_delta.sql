/*
Capture before/after P21 data changes across the full manual container workflow.
...
BEGIN
    ...
    ORDER BY
          CASE
              ...
              WHEN COALESCE(b.entity_name, a.entity_name) = 'transfer_backorders' THEN 120
              WHEN COALESCE(b.entity_name, a.entity_name) = 'anticipated_allocation' THEN 130
              WHEN COALESCE(b.entity_name, a.entity_name) = 'oe_line' THEN 140
              WHEN COALESCE(b.entity_name, a.entity_name) = 'inv_loc' THEN 150
              ELSE 999
          END
...
FROM dbo.container_building AS cb
WHERE
  (
...
FROM dbo.container_receipts_hdr AS crh
...
SELECT
    ...
    , cbp.po_line_uid
FROM dbo.container_building_po AS cbp
WHERE EXISTS
(
    SELECT 1
    FROM #scope_container_building AS cb
...
INNER JOIN dbo.transfer_line AS tl
...
WHERE (th.from_location_id = @TargetLocationId OR th.to_location_id = @TargetLocationId)
  ...
  (
      ...
      (
          ...
          AND th.date_created BETWEEN DATEADD(day, -@TransferSearchDaysBefore, P21.dbo.p21_fn_GetSystemDatetime(CURRENT_TIMESTAMP, NULL, NULL))
                                  AND DATEADD(day, @TransferSearchDaysAfter, P21.dbo.p21_fn_GetSystemDatetime(CURRENT_TIMESTAMP, NULL, NULL))
      )
  )
GROUP BY
      th.transfer_no;
...
(
    ...
    FROM dbo.transfer_backorders AS tb
    WHERE EXISTS
    (
        SELECT 1
        FROM #near_transfer AS nt
...
(
    ...
    FROM dbo.anticipated_allocation AS aa
