/*
================================================================================
PROPOSAL — Backfill missing Inventory Receipts (IR) GL postings for ADF-created
Coats vessel receipts in PRODUCTION P21.

DO NOT RUN THIS BLIND. This writes to the production general ledger. It is a
proposal for review + accounting sign-off, and should be tested in P21Play /
P21Import FIRST (both carry the same t_gl_iu insert trigger).
================================================================================
...
DECLARE @Now              datetime     = P21.dbo.p21_fn_GetSystemDatetime(CURRENT_TIMESTAMP, NULL, NULL);
...
UPDATE t
   SET t.vessel_name = vh.vessel_name,
       t.amount      = amt.amount
FROM #target AS t
INNER JOIN P21.dbo.vessel_receipts_hdr AS vh ON vh.vessel_receipts_hdr_uid = t.hdr_uid
...
BEGIN TRAN;
    ...
    EXEC P21.dbo.p21_get_counter @strCounterID = 'gl',
         @iIncrementValue = @glCount, @LastValue = @glLast OUTPUT;
    SET @glFirst = @glLast - @glCount + 1;

    EXEC P21.dbo.p21_get_counter @strCounterID = 'gl_transaction_number',
         @iIncrementValue = @N, @LastValue = @txnLast OUTPUT;
    SET @txnFirst = @txnLast - @N + 1;

    -- Safety: allocated blocks must be ahead of existing data
    IF @glFirst <= (SELECT ISNULL(MAX(gl_uid),0) FROM P21.dbo.gl)
        THROW 60004, 'Allocated gl_uid block overlaps existing rows (counter behind). Rolled back.', 1;
    IF @txnFirst <= (SELECT ISNULL(MAX(transaction_number),0) FROM P21.dbo.gl)
        THROW 60005, 'Allocated transaction_number block overlaps existing rows (counter behind). Rolled back.', 1;

    -- Ensure a dbo.balances row exists for each account we post to, provisioned
    -- exactly as P21's own vessel-receipt posting does (verified via capture DIFF 6->7):
    --   cumulative_balance = latest prior-period cumulative (carry-forward; 0 if none)
    --   period_balance     = 0
    -- This satisfies fk_gl_balances; the t_gl_iu trigger then rolls the posted
    -- amounts into these rows. Only missing rows are created (idempotent).
    -- NOTE: carry-forward inherits whatever the prior period holds. If the prior
    -- period is itself wrong (e.g. June missing unposted vessel accruals), that error
    -- carries forward -- an accounting-period decision, separate from this FK fix.
    INSERT INTO P21.dbo.balances
    (
    ...
    SELECT
          @CompanyNo, acct.account_no, @TargetPeriod, @TargetYear, 1,
          COALESCE(carry.cumulative_balance, 0), 0,
          0, 0, 0,
          'N', @Now, @Now, @Operator,
          0, 0, CONVERT(datetime, '1980-01-01'),
          0, 0, 0
    FROM (VALUES (@APAccount), (@InTransitAccount)) AS acct(account_no)
    OUTER APPLY
    (
        SELECT TOP (1) b.cumulative_balance
        FROM P21.dbo.balances AS b
        WHERE b.company_no = @CompanyNo
          AND b.account_no = acct.account_no
          AND b.currency_id = 1
          AND (b.year_for_period < @TargetYear
               OR (b.year_for_period = @TargetYear AND b.period < @TargetPeriod))
        ORDER BY b.year_for_period DESC, b.period DESC
    ) AS carry
    WHERE NOT EXISTS
    (
        SELECT 1 FROM P21.dbo.balances AS x
        WHERE x.company_no = @CompanyNo
          AND x.account_no = acct.account_no
          AND x.period = @TargetPeriod
          AND x.year_for_period = @TargetYear
          AND x.currency_id = 1
    );

    INSERT INTO P21.dbo.gl
    (
