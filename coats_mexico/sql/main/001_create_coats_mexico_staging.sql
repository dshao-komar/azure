...
BEGIN
    ...
    DECLARE @receipt_build_count int = 0;

    IF OBJECT_ID('dbo.coats_mexico_shipment_receipt_build', 'U') IS NOT NULL
    BEGIN
        EXEC sp_executesql
              N'
                SELECT @receipt_build_count = COUNT(*)
                FROM dbo.coats_mexico_shipment_receipt_build WITH (UPDLOCK, HOLDLOCK)
                WHERE shipment_file_id = @shipment_file_id;
              '
            , N'@shipment_file_id uniqueidentifier, @receipt_build_count int OUTPUT'
            , @shipment_file_id = @shipment_file_id
            , @receipt_build_count = @receipt_build_count OUTPUT;

        IF @receipt_build_count > 0
        BEGIN
            THROW 50002, 'Receipt records already exist for this shipment_file_id. Staging refresh is blocked.', 1;
        END;
    END;

    IF EXISTS (
        SELECT 1
        FROM dbo.coats_mexico_shipment_file WITH (UPDLOCK, HOLDLOCK)
        WHERE shipment_file_id = @shipment_file_id
    )
    BEGIN
    ...
    DELETE FROM dbo.coats_mexico_shipment_validation_issue
    WHERE shipment_file_id = @shipment_file_id;

    DELETE FROM dbo.coats_mexico_shipment_pallet_line
    WHERE shipment_file_id = @shipment_file_id;

    DELETE FROM dbo.coats_mexico_shipment_raw_line
    WHERE shipment_file_id = @shipment_file_id;
