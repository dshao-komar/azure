...
CREATE OR ALTER PROCEDURE dbo.usp_create_coats_mexico_p21import_receipts
      @ShipmentFileId uniqueidentifier
...
BEGIN
    ...
    VALUES
        ...
        , ('vessel_receipts_line', 'qty_ordered')
        , ('vessel_receipts_line', 'qty_received')
        , ('vessel_receipts_line', 'unit_of_measure')
        , ('vessel_receipts_line', 'unit_size')
    ...
    BEGIN
        SELECT
              rc.table_name
            , rc.column_name
            , issue = 'Missing required P21Import column'
        FROM @required_column AS rc
        WHERE COL_LENGTH('dbo.' + rc.table_name, rc.column_name) IS NULL
        ORDER BY
              rc.table_name
